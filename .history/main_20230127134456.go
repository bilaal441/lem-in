...
func (g *Graph) sendAnts(numAnts int, paths [][]string, pathsWithAnts map[int][]int) {
	...
	timeMoved := make([]int, numAnts)
	for i := range timeMoved {
		timeMoved[i] = -1
	}
	for time := 0; remainingAnts > 0; time++ {
		for i := 0; i < numAnts; i++ {
			path, ok := getPathOfAnt(i, pathsWithAnts)
			if !ok {
				continue
			}
			if timeMoved[i] < time && currentPos[i] < len(paths[path]) {
				fmt.Printf("L%d-%s ", i+1, paths[path][currentPos[i]])
				if paths[path][currentPos[i]] == "end" {
					remainingAnts--
				}
				currentPos[i]++
				timeMoved[i] = time
...
func getPathOfAnt(ant int, pathsWithAnts map[int][]int) (int, bool) {
