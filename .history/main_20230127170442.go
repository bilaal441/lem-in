...
func sendAnts(paths [][]string, ants int, pathAnt map[int]int) {
	// Initialize a map to store the position of each ant on its path
	antPos := make(map[int]int)
	for i := 1; i <= ants; i++ {
		// Set the initial position of each ant to the start of its path
		antPos[i] = 0
	}

	for {
		// Keep track of whether any ants have reached the end of their paths
		allAntsReachedEnd := true

		// Iterate through the paths
		for i, path := range paths {
			// Iterate through the position of each ant on this path
			for j := 0; j < len(path); j++ {
				// Check if this position contains an ant
				for k := 1; k <= ants; k++ {
					// Check if this ant is on this path and at this position
					if pathAnt[k] == i && antPos[k] == j {
						// Print the ant's current position
						fmt.Printf("L%d-%s ", k, path[j])

						// Update the ant's position
						antPos[k]++

						// Check if the ant has reached the end of its path
						if antPos[k] == len(path) {
							// Print "end" to indicate that the ant has reached the end
							fmt.Printf("L%d-end ", k)
						} else {
							// Set the flag to indicate that not all ants have reached the end
							allAntsReachedEnd = false
						}
					}
				}
			}
