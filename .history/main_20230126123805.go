...
func sendAnts(paths [][]string, pathWithAnts map[int][]int) {
	// Find the maximum number of steps needed to traverse all paths
	maxSteps := 0
	...
	for step := 0; step < maxSteps; step++ {
		...
		for pathIndex, path := range paths {
			// Loop over each ant on the current path
			for _, ant := range pathWithAnts[pathIndex] {
				// If the ant has not reached the end of the path yet
				if step < len(path)-1 {
					fmt.Printf("L%d-%s ", ant, path[step])
				} else {
