...
func assignPaths(numAnts int, paths [][]string) map[int][]int {
	...
	currentPath := 0
	for i := 0; i < numAnts; i++ {
		pathWithAnts[currentPath] = append(pathWithAnts[currentPath], i+1)
		if currentPath+1 < len(paths) &&
			(len(pathWithAnts[currentPath])+len(paths[currentPath])-1) >
				(len(pathWithAnts[currentPath+1])+len(paths[currentPath+1])-1) {
			currentPath++
		}
	}
	return pathWithAnts
