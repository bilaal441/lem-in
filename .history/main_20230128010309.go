...
func sendAnts(paths [][]string, ants int, pathAnt map[int]int) {
	...
	for i, p := range paths {
		...
		tPaths.rooms = append(p, p...)
		tPaths.ants = pathAnt[i]

	}
	fmt.Println(finalpaths[i].rooms, "hello")

	finalpaths = append(finalpaths, &tPaths)
	finalpaths[0].firstAnt = 1
	if len(finalpaths) > 1 {
		for i := 1; i < len(finalpaths); i++ {
			
			finalpaths[i].firstAnt = finalpaths[i-1].firstAnt + finalpaths[i-1].ants
		}
	}

	// for i := 1; ants > 0; i++ {
	// 	for _, path := range finalpaths {
	// 		for k := path.firstAnt; k <= path.ants+path.firstAnt-1; k++ {

	// 			l := i - k + path.firstAnt
	// 			fmt.Println(path.rooms)
	// 			if l > 0 && l < len(path.rooms) {
	// 				// fmt.Println(i - k + pathAnt[j][0])
	// 				fmt.Printf("L%v-%v ", i, path.rooms[l])

	// 			}
	// 			if l == len(path.rooms)-1 {
	// 				ants--
	// 			}
