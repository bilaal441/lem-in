...
func sendAnts(paths [][]string, ants int, pathAnt map[int]int) {
	// Initialize an array to keep track of the ants that have reached the end of each path
	end := make([]int, len(paths))
	for i := 1; ants > 0; i++ {
		// Iterate through each path
		for j, path := range paths {
			// Only move the ants that haven't reached the end yet
			for k := 0; k < pathAnt[j]-end[j]; k++ {
				// Find the next available index for the ant
				j := i - end[j] - k
				if j > 0 && j < len(path) {
					fmt.Printf("L%v-%v ", i, path[j])
				}
				if j == len(path)-1 {
					end[j]++
					ants--
