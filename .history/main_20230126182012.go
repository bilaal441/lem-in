...
func (g *Graph) sendAnts(paths [][]string, pathsWithAnts map[int][]int) {
	...
	for stepIndex := range paths[0] {
		for pathIndex, antList := range pathsWithAnts {
			if stepIndex < len(paths[pathIndex]) {
				for _, ant := range antList {
					if paths[pathIndex][stepIndex] != paths[pathIndex][len(paths[pathIndex])-1] {
            fmt.Println( paths[pathIndex][len(paths[pathIndex])-1])
						fmt.Printf("L%d-%s ", ant, paths[pathIndex][stepIndex])
					}
				}
			}
		}
