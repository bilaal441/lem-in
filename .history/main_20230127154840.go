...
func (g *Graph) sendAnts(numAnt int, paths [][]string, AntsEachPath map[int]int) {
	positions := make(map[int]int) // keeps track of the current position of each ant
	antsReachedEnd := 0 // keeps track of the number of ants that have reached the end of the path
	for i := 1; i <= numAnt; i++ {
			positions[i] = 0 // set initial position of each ant to 0
	}
	for {
			for _, path := range paths {
					...
					fmt.Println("end")
					time.Sleep(1000 * time.Millisecond) // delay of 1 second between each move
			}
			if antsReachedEnd == numAnt {
					break
			}
	}
}
