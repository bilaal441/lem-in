...
func (g *Graph) sendAnts(numAnts int, paths [][]string, pathsWithAnts map[int][]int) {
	...
	// Create a temporary array to store the ants
	temp := make([][]string, numAnts)

	for path, ants := range pathsWithAnts {
		for _, ant := range ants {
			if ant >= len(currentPos) {
				continue
			}
			if currentPos[ant] < len(paths[path]) {
				temp[ant] = []string{fmt.Sprintf("%d", ant+1), paths[path][currentPos[ant]]}
	...
	// Sort the temporary array based on the current position of the ants
	sort.Slice(temp, func(i, j int) bool {
		return currentPos[i] < currentPos[j]
	})

	// Print the ants in the sorted order
	for _, ant := range temp {
		fmt.Printf("L%s-%s ", ant[0], ant[1])
	}
	fmt.Println()
