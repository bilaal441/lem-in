...
func sendAnts(paths [][]string, ants int, pathAnt map[int]int) {
	...
	antCounter := 1
	for path, antCount := range pathAnt {
		for i := 0; i < antCount; i++ {
			paths[path][i] = strconv.Itoa(antCounter)
			antCounter++
		}
	}

	// Iterate through paths and rooms
	for row := 1; ants > 0; row++ {
		for i, path := range paths {
			for j, room := range path {
				if room != "" {
					antNum, _ := strconv.Atoi(room)
					if row == 1 {
						fmt.Printf("L%v-%v ", antNum, "t")
					} else if row-1 < len(paths[i]) {
						fmt.Printf("L%v-%v ", antNum, paths[i][row-1])
					} else {
						fmt.Printf("L%v-%v ", antNum, "end")
						ants--
					}
				}
			}
...
func main() {
	...
	sendAnts(paths, numAnts, assignPaths(assignPaths()[]))
