...
func assignPaths(numAnts int, paths [][]string) map[int][]int {
	...
	pathWithAnts[paths[0]] = append(pathWithAnts[0], 1)
	for i := 1; i <= numAnts-1; i++ {
		fmt.Println(i)
		for current, path := range paths {
			if current+1 < len(paths) && len(pathWithAnts[current])+len(path) > len(pathWithAnts[current+1])+len(paths[current+1]) {
				// fmt.Println(path, i)
				...
				pathWithAnts[current+1] = append(pathWithAnts[current], i)
			}
