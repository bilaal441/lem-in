...
func PrintTurns(paths []*Path, ants int) {
	maxTurns := 0
	for _, path := range paths {
		if len(path.Rooms) > maxTurns {
			maxTurns = len(path.Rooms)
		}
	}
	for turn := 1; turn <= maxTurns; turn++ {
		for _, path := range paths {
			if turn <= len(path.Rooms) {
				if turn == 1 {
					antsInPath := min(ants, path.Ants)
					ants -= antsInPath
					for i := 1; i <= antsInPath; i++ {
						fmt.Printf("L%v-%v ", i, path.Rooms[turn-1].Name)
					}
				} else {
					for i := 1; i <= path.Ants; i++ {
						if path.Rooms[turn-2].Name != path.Rooms[turn-1].Name {
							fmt.Printf("L%v-%v ", i, path.Rooms[turn-1].Name)
						}
					}
				}
