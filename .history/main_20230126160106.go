...
func sendAnts(paths [][]string, pathsWithAnts map[int][]int) {
	...
	for {
		...
		for pathIndex, ants := range pathsWithAnts {
			for i, ant := range ants {
				fmt.Println(antLocations[ant], paths[pathIndex][len(paths[pathIndex])-1])
				if antLocations[ant] != paths[pathIndex][len(paths[pathIndex])-1] {
					allAntsArrived = false
					if pathCount[pathIndex] == i {
