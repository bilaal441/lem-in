...
func (g *Graph) sendAnts(numAnts int, paths [][]string, pathsWithAnts map[int][]int) {
	...
	finishedAnts := make([]bool, numAnts)

	for time := 0; remainingAnts > 0; time++ {
		finishedInThisStep := 0
		for i := range currentPos {
			if !finishedAnts[i] {
				path := paths[pathsWithAnts[i]]
				currentNode := paths[path][currentPos[i]]
				if currentNode == "end" {
					finishedAnts[i] = true
					finishedInThisStep++
				} else {
					fmt.Printf("L%d-%s ", i+1, currentNode)
					currentPos[i]++
				}
			} else {
				finishedInThisStep++
		...
		remainingAnts -= finishedInThisStep
		fmt.Println()
	}
}
