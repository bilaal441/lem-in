...
func sendAnts(paths [][]string, pathsWithAnats map[int][]int) {
	// Create a map of ants waiting to move to each room
	waitingAnts := make(map[string][]int)
	for i, path := range paths {
		for _, antID := range pathsWithAnats[i] {
			waitingAnts[path[1]] = append(waitingAnts[path[1]], antID)
		}
	}
	...
	for i := 1; i < len(paths[0]); i++ {
		for _, path := range paths {
			// Check if there are any ants waiting to move to the current room
			if ants, ok := waitingAnts[path[i]]; ok {
				for _, antID := range ants {
					fmt.Printf("L%d-%s ", antID, path[i])
				}
				// Remove the ants that have moved to the current room
				delete(waitingAnts, path[i])
			}
		}
		fmt.Println()
	}
