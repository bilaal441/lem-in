...
func sendAnts(paths [][]string, pathsWithAnats map[int][]int) {
	// Create a map to store ants that have reached the end
	endAnts := make(map[int]bool)
	// Get the total number of turns
	totalTurns := len(paths[0]) + len(paths[1]) + len(paths[2])
	for turn := 0; turn < totalTurns; turn++ {
			for pathIndex, path := range paths {
					for i, room := range path {
							// Check if there is an ant in the current room
							if len(pathsWithAnats[pathIndex]) > i {
									ant := pathsWithAnats[pathIndex][i]
									...
									if i+1 < len(path) {
											pathsWithAnats[pathIndex][i] = pathsWithAnats[pathIndex][i+1]
									} else {
											pathsWithAnats[pathIndex] = pathsWithAnats[pathIndex][i+1:]
									}
									// Print the ant's current location
									fmt.Printf("L%d-%s ", ant, room)
							}
					}
			}
			// Check if all ants have reached the end
			if len(endAnts) == len(pathsWithAnats) {
					break
			}
			fmt.Println()
	}
