...
func sendAnts(paths [][]string, pathWithAnts map[int][]int) {
	// Create a map to keep track of the current room for each ant
	antsInRooms := make(map[int]string)
	for i, path := range paths {
		for _, ant := range pathWithAnts[i] {
			antsInRooms[ant] = path[0]
		}
	}

	// Initialize a variable to keep track of whether any ants are still moving
	antsStillMoving := true

	// Loop until all ants have reached the end
	for antsStillMoving {
		antsStillMoving = false
		for i, path := range paths {
			for j, ant := range pathWithAnts[i] {
				if antsInRooms[ant] != "end" {
					// Print the current room for the ant
					fmt.Printf("L%d-%s ", ant, antsInRooms[ant])

					// Update the current room for the ant
					antsInRooms[ant] = path[j+1]

					// Check if the ant has reached the end
					if antsInRooms[ant] == "end" {
						fmt.Printf("L%d-end ", ant)
					} else {
						antsStillMoving = true
					}
				}
