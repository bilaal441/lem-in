	...
	for time := 0; remainingAnts > 0; time++ {
		for path, ants := range pathsWithAnts {
			for i, ant := range ants {
				if time >= currentPos[i] {
					if currentPos[i] < len(paths[path]) {
						fmt.Printf("L%d-%s ", ant, paths[path][currentPos[i]])
					}
					fmt.Println(currentPos[i])
					if currentPos[i] < len(paths[path])&& paths[path][currentPos[i]] == "end" {
						remainingAnts--
					}
					currentPos[i]++
				}
