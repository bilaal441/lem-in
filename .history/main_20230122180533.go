...
func assignPaths(numAnts int, paths [][]string) map[int][]int {
	...
	pathWithAnts[0] = append(pathWithAnts[0], 1)
	currentpath := pathWithAnts[0]
	for i := 1; i <= numAnts; i++ {
		// fmt.Println("i", i)
		for current, path := range paths {
			rooms := len(path[1 : len(path)-1])
			// next := len(paths[current+1][1 : len(paths[current+1])-1])

			if current+1 < len(paths) && len(currentpath)+rooms > len(pathWithAnts[i+1])+len(paths[current+1][1:len(paths[current+1])-1]) {
				// fmt.Println(len(paths[current][1:len(paths[current])-1]) + len(pathWithAnts[current]))
				pathWithAnts[current+1] = append(pathWithAnts[current+1], i)

			}
			currentpath = pathWithAnts[current+1]
...
func main() {
	...
	paths := data.getPaths()
	fmt.Println(numAnts)
	fmt.Println(assignPaths(numAnts, paths))
