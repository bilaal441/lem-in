...
func assignPaths(numAnts int, paths []*path) map[int]int {
	...
	for i := 1; i <= numAnts; i++ {
		...
		if currentPath+1 < len(paths) &&
			pathWithAnts[currentPath]+len(path[currentPath].) >
				pathWithAnts[currentPath+1]+len(path[currentPath+1]) {
