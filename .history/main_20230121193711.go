...
func (graph *Graph) FindAugmentingPath() ([]*Vertex, bool) {
	...
	for len(queue) > 0 {
		...
		for _, v := range current.Adj {
			...
			if edge.Flow-edge.capacity < 0 && !visited[v] && v.RoomCapacity > 2 {
...
func main() {
	...
	for _, curr := range data.GetVertex("h").Adj {
		b := data.getEdge(data.GetVertex("h"), curr)
		fmt.Println(b.Flow, curr.Name)
	}
