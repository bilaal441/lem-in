...
func (g *Graph) getPaths() [][]string {
	...
	for len(queue) > 0 {
		...
		if Clone(path)[lastNode] == goalNode {
			...
			result = append(result, Clone(path)[1:])
			...
			for _, nbr := range g.GetVertex(Clone(path)[lastNode]).Adj {
				...
				v := g.getEdge(g.GetVertex(Clone(path)[lastNode]), nbr)
				// fmt.Println(v.Flow, "current", Clone(path)[lastNode], "neib", nbr.Name)
				if isNotVisited(Clone(path), nbr.Name) && v.Flow == v.capacity {
