...
func sendAnts(paths [][]string, pathsWithAnats map[int][]int) {
	...
	steps := len(paths[len(paths)-1]) // number of steps required for all ants to reach the end

	for i := 0; i < steps; i++ {
		for j, path := range paths {
			if i < len(path) {
				for _, antID := range pathsWithAnats[j] {
					if i == len(path)-1 {
						fmt.Printf("L%d-end ", antID)
					} else {
						fmt.Printf("L%d-%s ", antID, path[i][1:])
					}
				}
			}
		}
