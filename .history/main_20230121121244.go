...
func checkPathOverlap(paths [][]*Vertex) bool {
	for i := 0; i < len(paths); i++ {
		for j := i + 1; j < len(paths); j++ {
			for _, vertex1 := range paths[i][1 : len(paths[i])-1] {
				for _, vertex2 := range paths[j][1 : len(paths[j])-1] {
					if vertex1.Name == vertex2.Name {
						fmt.Println(vertex2.Name, vertex1.Name)
						return true
					}
				}
			}
...
func (data *Graph) findWrightPaths() {
   if checkPathOverlap(lem_in(data)){
		
	 }
	fmt.Println(checkPathOverlap(lem_in(data)))
