...
func checkPathOverlap(paths [][]*Vertex) bool {
	for i := 0; i < len(paths); i++ {
		for j := i + 1; j < len(paths); j++ {
			for _, vertex1 := range paths[i] {
				for _, vertex2 := range paths[j] {
					if vertex1.Name == vertex2 {
						return true
					}
				}
			}
...
func findWrightPaths(paths [][]string) [][]string {
	if len(paths) < 2 {
		return paths
	}
...
func main() {
	...
	// residualNetwork := data.InitializeResidualNetwork(links)
	...
	// for _, curr := range data.GetVertex("n").edges {
