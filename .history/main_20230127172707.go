...
func sendAnts(paths [][]string, ants int, pathAnt map[int]int) {
	fmt.Println()
	// Create a 2D array to represent the ants' positions on each level
	positions := make([][]int, len(paths))
	for i := range positions {
		positions[i] = make([]int, len(paths[i]))
	}

	// Initialize the ants' starting positions
	for i := 0; i < ants; i++ {
		positions[0][0]++
	}

	// Iterate through the array, updating the ants' positions
	for i := 0; i < len(paths); i++ {
		for j := 0; j < len(paths[i]); j++ {
			// Move the ants to the next level
			positions[i][j+1] += positions[i][j]
			positions[i+1][j] += positions[i][j]

			// Print the ants' positions
			for k := 0; k < positions[i][j]; k++ {
				fmt.Printf("L%d-%s ", k+1, paths[i][j])
			}
		}
		fmt.Println("")
	}
