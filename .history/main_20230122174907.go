...
func assignPaths(numAnts int, paths [][]string) map[int][]int {
	...
	for i := 1; i <= numAnts-1; i++ {
		// fmt.Println("i", i)
		for current, _ := range paths {
			if current+1 < len(paths) && len(paths[current][1:len(paths[current])-1])+len(pathWithAnts[current][i]) > len(paths[current+1][1:len(paths[current+1])-1]) {
				fmt.Println(len(paths[current][1:len(paths[current])-1]) + len(pathWithAnts[current]))
				pathWithAnts[current+1] = append(pathWithAnts[current+1], i+1)
