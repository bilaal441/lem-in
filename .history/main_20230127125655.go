...
func (g *Graph) sendAnts(numAnts int, paths [][]string, pathsWithAnts map[int][]int) {
	...
	for step := 0; remainingAnts > 0; step++ {
		for path, ants := range pathsWithAnts {
			for _, ant := range ants {
				...
				if currentPos[ant] < len(paths[path]) {
					currentPos[ant]++
					if paths[path][currentPos[ant]-1] == "end" {
						remainingAnts--
					}
				}
			}
		}
		for i, pos := range currentPos {
			if pos > 0 && pos < i < len(for i, pos := range currentPos {
				if pos > 0 && pos < len(paths[pathsWithAnts[i][0]]) {
					fmt.Printf("L%d-%s ", i+1, paths[pathsWithAnts[i][0]][pos-1])
				}
			}
	) {
				fmt.Printf("L%d-%s ", i+1, paths[pathsWithAnts[i][0]][pos-1])
