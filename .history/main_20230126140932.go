...
func sendAnts(paths [][]string, pathsWithAnats map[int][]int) {
	antsPos := make(map[int]int)
	for _, path := range pathsWithAnats {
		for _, antID := range path {
			antsPos[antID] = 0 // start position of the ant
		}
	}
	for {
		moved := false
		for antID, pos := range antsPos {
			if pos < len(pathsWithAnats[antID])-1 {
				moved = true
				antsPos[antID]++
				fmt.Printf("L%d-%s ", antID, paths[antID][pos+1])
			}
		}
		if !moved {
			break
